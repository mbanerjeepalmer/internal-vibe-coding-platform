...
export interface SandboxProvider {
	getOrCreateSandbox(projectId: string, environment?: Record<string, string>): Promise<Sandbox>;
...
class DaytonaSandboxProvider implements SandboxProvider {
	...
	private async ensureProjectScaffold(daytonaSandbox: { process: DaytonaProcess }, projectId: string) {
		...
		if (check.result?.includes('present')) {
			...
			const result = await daytonaSandbox.process.executeCommand(
				`cd ${PROJECT_DIR} && mkdir -p .opencode/skills/vibe-app-storage .opencode/skills/vibe-app-secrets .opencode/tools && test -f .opencode/skills/vibe-app-storage/SKILL.md || printf '%s' '${storageGuide}' > .opencode/skills/vibe-app-storage/SKILL.md; test -f .opencode/skills/vibe-app-secrets/SKILL.md || printf '%s' '${secretsGuide}' > .opencode/skills/vibe-app-secrets/SKILL.md; test -f .opencode/tools/request_secret.ts || printf '%s' '${requestTool}' > .opencode/tools/request_secret.ts; grep -q 'vibe-app-storage' AGENTS.md || printf '\n\nFor database, storage, D1, SQL, schema, migration, persistence, backup, restore, relink, or deletion work, load the \`vibe-app-storage\` skill.\n' >> AGENTS.md; grep -q 'vibe-app-secrets' AGENTS.md || printf '\n\nFor credentials, API keys, tokens, passwords, secrets, or environment variables, load the \`vibe-app-secrets\` skill.\n' >> AGENTS.md`
			);
		...
		const writes = Object.entries(files)
			...
			.join(' && ');
		const result = await daytonaSandbox.process.executeCommand(
			`mkdir -p ${PROJECT_DIR}/migrations ${PROJECT_DIR}/.opencode/skills/vibe-app-storage ${PROJECT_DIR}/.opencode/skills/vibe-app-secrets ${PROJECT_DIR}/.opencode/tools && ${writes}`
		);
